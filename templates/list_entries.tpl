...
<div class="admin nlpg">
	<div class="header">
		<h1>{tr}NLPG Extracts{/tr}</h1>
	</div>
	...
	<div class="body">
		...
		{form id="checkform"}
			...
			<table class="data">
				<caption>{tr}NLPG Loaded Extracts and Updates{/tr} <span class="total">[ {$listInfo.total_records} ]</span></caption>
				<tr>
					{*  at the moment, the only working option to use the checkboxes for is deleting pages. so for now the checkboxes are visible iff $p_nlpg_remove_page is set. Other applications make sense as well (categorize, convert to pdf, etc). Add necessary corresponding permission here: *}

					{if $gBitUser->hasPermission( 'p_nlpg_remove_entry' )} 
						{assign var='checkboxes_on' value='y'}
					{else}
						{assign var='checkboxes_on' value='n'}
					{/if}
					{counter name=cols start=-1 print=false}
					{if $gBitSystem->isFeatureActive( 'nlpg_list_lastver' )}
						<th>{smartlink ititle="Last Version" isort="version" icontrol=$listInfo}</th>
						{counter name=cols assign=cols print=false}
					{/if}
					{if $gBitSystem->isFeatureActive( 'nlpg_list_comment' )}
				...
				{section name=changes loop=$listpages}
					<tr class="{cycle advance=false}">
						<td colspan="{$cols}">
							...
							{if $gBitSystem->isFeatureActive( 'nlpg_list_creator' ) && $gBitSystem->isFeatureActive( 'nlpg_list_lastmodif' ) }
							...
							{else}
								...
								{/if}
								, {$listpages[changes].created|bit_short_datetime}
								{if $gBitSystem->isFeatureActive( 'nlpg_list_lastmodif' ) && ($listpages[changes].version > 1)}
									<br />
									{tr}Last modified{/tr}
					...
					<tr class="{cycle}">
						{if $gBitSystem->isFeatureActive( 'nlpg_list_hits' )}
						...
						{/if}
						{if $gBitSystem->isFeatureActive( 'nlpg_list_versions' )}
							{if $gBitSystem->isFeatureActive( 'nlpg_history' )}
						...
						{/if}
						{if $gBitSystem->isFeatureActive( 'nlpg_list_backlinks' )}
							{if $gBitSystem->isFeatureActive( 'nlpg_backlinks' ) && $listpages[changes].backlinks > 0}
								<td style="text-align:center;"><a href="{$smarty.const.nlpg_PKG_URL}backlinks.php?page={$listpages[changes].title|escape:"url"}">{$listpages[changes].backlinks|default:"0"}</a></td>
							{else}
			...
			{if $checkboxes_on eq 'y'}
				<div style="text-align:right;">
					...
					<select name="batch_submit" onchange="this.form.submit();">
						...
						{if $gBitUser->hasPermission( 'p_nlpg_remove_page' )}
